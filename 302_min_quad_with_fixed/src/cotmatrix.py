...
def cotmatrix(V,F,regularization=1e-6):
    '''
    ...
    for i in range(3):
        ...
        norm_temp1 = np.sqrt(np.power(temp1,2).sum(axis = 1)) + regularization
        norm_temp2 = np.sqrt(np.power(temp2,2).sum(axis = 1)) + regularization
